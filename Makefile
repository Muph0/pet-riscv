...
list:
	...
	if $(SINGLE_TB); then rm -rf build; fi

sim_uart_rx:
	@echo SINGLE_TB is $(SINGLE_TB)
...
sim_cpu_memtest: rust/memtest.bin
...
test_all:
	make SINGLE_TB=false sim_uart_rx sim_cpu_boot_decode sim_cpu_fwd sim_cpu_lw sim_cpu_fib sim_cpu_uart_echo sim_cpu_boot_echo sim_bus_xbar sim_bus_xbar_atomic
	rm -rf build
